...
DATASET2PATH = {
    ...
    "RAWFC_poisoned": os.path.join(HOME_DIR + "dataset/RAWFC_poisoned/"),
...
LABEL_IDS = {
    ...
    "RAWFC_poisoned": {"false": 0, "half": 1, "true": 2},
...
class Stage2PredictionDatasetForRoBERTa(Dataset):
    ...
    def format(self):
        ...
        for i, claim in tqdm(enumerate(self.claims)):
            true_exp = explanations[2 * i].get("explanation", "")
            false_exp = explanations[2 * i + 1].get("explanation", "")
            if self.do_filter:
